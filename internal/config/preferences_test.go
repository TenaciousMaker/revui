...
func TestSaveLoadAndDefaults(t *testing.T) {
	...
	want := Preferences{FileLayout: "tree", FileScope: "all", WideFiles: true, DiffView: "split", IgnoreWhitespace: true, SemanticReflow: true, NormalizedLayout: true, DifftasticMode: true}
	...
	if got.FileLayout != want.FileLayout || got.FileScope != want.FileScope || !got.WideFiles || got.DiffView != want.DiffView || !got.IgnoreWhitespace || !got.SemanticReflow || !got.NormalizedLayout || !got.DifftasticMode {
